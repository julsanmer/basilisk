...
#ifndef SMALLBODY_NAV3_MESSAGE_H
#define SMALLBODY_NAV3_MESSAGE_H
...
typedef struct{
    double state[13];      //!< [units] state
    double covar[13][13];  //!< [units] covariance
    double skew[13];       //!< [units] skewness
    double kurt[13];       //!< [units] kurtosis
    double meas[3];        //!< [units] measurement
    double tcpu;           //!< [units] computation time
}SmallBodyNav3MsgPayload;
