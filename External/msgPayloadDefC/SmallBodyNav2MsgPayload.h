...
#ifndef SMALLBODY_NAV2_MESSAGE_H
#define SMALLBODY_NAV2_MESSAGE_H
...
typedef struct{
    double state[10];      //!< [units] state
    double covar[10][10];  //!< [units] covariance
    double skew[10];       //!< [units] skewness
    double kurt[10];       //!< [units] kurtosis
    double meas[3];        //!< [units] measurement
    double tcpu;           //!< [units] computation time
}SmallBodyNav2MsgPayload;
